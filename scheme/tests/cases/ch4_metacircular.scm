; Code from SICP chapter 4.1 (Abelson, Sussman, Sussman / MIT Press), CC BY-SA 4.0.
; See book/ATTRIBUTION.md. The metacircular evaluator, renamed my-eval/my-apply so it
; sits alongside the host's own eval and apply rather than shadowing them.
...
(define (sequence->exp seq)
  (cond ((null? seq) seq) ((last-exp? seq) (first-exp seq)) (else (make-begin seq))))
  ...
  (if (null? clauses) 'false
      (let ((first (car clauses)) (rest (cdr clauses)))
        (if (cond-else-clause? first)
            (if (null? rest) (sequence->exp (cond-actions first)) (error "ELSE clause isn't last" clauses))
...
(define (extend-environment vars vals base-env)
  (if (= (length vars) (length vals))
      ...
      (error "Wrong number of args supplied" vars)))
...
(define (my-eval exp env)
  (cond ((self-evaluating? exp) exp)
        ...
        ((application? exp) (my-apply (my-eval (operator exp) env) (list-of-values (operands exp) env)))
...
(my-eval '(define (fact n) (if (= n 1) 1 (* n (fact (- n 1))))) the-global-environment)
(my-eval '(fact 10) the-global-environment)
(my-eval '(cond ((= 1 2) 'no) (else 'yes)) the-global-environment)
(my-eval '((lambda (x y) (+ x y)) 3 4) the-global-environment)
(my-eval '(define (map2 f l) (if (null? l) (quote ()) (cons (f (car l)) (map2 f (cdr l))))) the-global-environment)
(my-eval '(map2 (lambda (x) (* x x)) (quote (1 2 3 4))) the-global-environment)
